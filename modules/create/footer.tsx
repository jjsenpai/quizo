...
export const Footer = () => {
  ...
  const selected = pathname.includes("/events") ? 1 : 0;

  return (
    <div className="h-20 w-full max-w-2xl p-2">
      <div className="flex h-full items-center gap-1 rounded-xl border-2 border-gray-200 bg-light p-1 text-gray-900">
        <Link
          href="/create"
          className={`inline-block h-full w-full justify-center rounded-lg ${selected === 0 ? `bg-white shadow` : `bg-light`} pb-1 pt-1.5 text-center`}>
          <svg width="25" height="25" viewBox="0 0 42 42" className="mb-1 inline-block">
        ...
        <Link
          href="/create/events"
          className={`inline-block h-full w-full justify-center rounded-xl pb-1 pt-1.5 text-center ${selected === 1 ? `bg-white shadow` : `bg-light`}`}>
          <svg width="25" height="25" viewBox="0 0 42 42" className="mb-1 inline-block">
            <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
              <path
