...
export interface IRequestOptions<TData extends object = object, TParams extends object = object, THeaders extends object = object> {
    url: string;
    data?: TData;
    params?: TParams;
...
export class FetchClient {
    ...
    private processUrl(url?: string) {
        return url ? this.config.baseURL!.replace(/\/?\/$/, "") + "/" + url.replace(/^\/+/, "") : this.config.baseURL;
    }
    ...
    private verifySession = async () => {
        ...
        if (!verifiedToken) {
            redirect(FRONTEND_URLS.LOGIN);
        }
    };

    private getOrDelete = async <TResponse, TParams extends object = object, THeaders extends object = object>(
        method: "GET" | "DELETE",
        ...
        const finalUrl = options.process === undefined || options.process ? this.processUrl(`${url}${this.getParamsString(params)}`) : url;
    ...
    private postOrPut = async <TResponse, TData extends object = object, TParams extends object = object, THeaders extends object = object>(
        method: "POST" | "PUT",
    ...
    public get = async <TResponse, TParams extends object = object, THeaders extends object = object>(
        options: IRequestOptions<object, TParams, THeaders>
    ) => {
        return await this.getOrDelete<TResponse>("GET", options);
    };

    public delete = async <TResponse, TParams extends object = object, THeaders extends object = object>(
        options: IRequestOptions<object, TParams, THeaders>
    ) => {
        return await this.getOrDelete<TResponse>("DELETE", options);
    };

    public post = async <TResponse, TData extends object = object, TParams extends object = object, THeaders extends object = object>(
        options: IRequestOptions<TData, TParams, THeaders>
    ) => {
        return await this.postOrPut<TResponse>("POST", options);
    };

    public put = async <TResponse, TData extends object = object, TParams extends object = object, THeaders extends object = object>(
        options: IRequestOptions<TData, TParams, THeaders>
    ) => {
